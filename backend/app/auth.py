...
class UserResponse(BaseModel):
    ...
    email: EmailStr
    created_at: datetime
    # The financial-profile `users.id` this account owns, if any has been
    # created yet (via POST /api/users). Lets the frontend recover the
    # correct financial profile from the authenticated backend account
    # instead of trusting only its local localStorage cache — e.g. after a
    # login from a new browser/device. Never another user's id: this is
    # always looked up by the caller's own verified auth_user_id.
    financial_user_id: Optional[int] = None
...
def get_me(current_user: User = Depends(get_current_user)):
    """Logged-in user ki apni profile info, plus the id of the financial
    profile this account owns (if one exists yet) — looked up strictly by
    the caller's own verified id, never anyone else's."""
    financial_user_id = None
    try:
        # Deferred imports: keeps this module self-contained for anyone who
        # only wants signup/login/get_current_user, per the module docstring.
        from sqlmodel import Session as SQLModelSession, select as sqlmodel_select
        from app.database import engine as financial_engine
        from app.models import User as FinancialUser

        with SQLModelSession(financial_engine) as financial_session:
            financial_user = financial_session.exec(
                sqlmodel_select(FinancialUser).where(
                    FinancialUser.auth_user_id == current_user.id
                )
            ).first()
            if financial_user:
                financial_user_id = financial_user.id
    except Exception:
        # Best-effort enrichment only — /auth/me must still succeed even if
        # the financial DB/table isn't reachable for some reason.
        financial_user_id = None

    response = UserResponse.model_validate(current_user)
    response.financial_user_id = financial_user_id
    return response
